...
const services = [
  ...
  {
    name: 'Infra Automation (Terraform/Ansible)',
    href: '/services/automation-tooling/infra-automation',
    description: 'Infrastructure as Code with Terraform and Ansible for cloud and on-premises with policy-as-code and compliance checks.',
  },
  {
    name: 'Network Automation',
    href: '/services/automation-tooling/network-automation',
    description: 'Configuration management, change validation, rollback automation, and integration with network orchestration platforms.',
  },
  {
    name: 'Custom Developer Tools',
    href: '/services/automation-tooling/custom-dev-tools',
    description: 'Development of specialized automation tools, APIs, and integrations tailored to specific operational requirements.',
  },
]

export default function AutomationToolingPage() {
  const caseStudiesRef = useRef<HTMLDivElement | null>(null)
  
  // Filter case studies relevant to Automation & Tooling
  const automationTags = ['Automation', 'CI/CD', 'DevOps', 'Terraform', 'Ansible', 'GitOps', 'Infrastructure', 'Network Automation', 'Tooling']
  const caseStudiesPreview = (caseStudies || []).filter(study => 
    (study.tags || []).some(tag => automationTags.some(autoTag => tag.toLowerCase().includes(autoTag.toLowerCase()))) ||
    (study.title || '').toLowerCase().includes('automation') ||
    (study.title || '').toLowerCase().includes('cicd') ||
    (study.summary || '').toLowerCase().includes('automation') ||
    (study.summary || '').toLowerCase().includes('terraform')
  ).slice(0, 8)
  ...
  return (
    <main className={styles.categoryPage}>
      <section className={styles.hero}>
        <div className="container-custom">
          ...
          <div className={styles.heroContent}>
            <h1 className="heading-1">Automation & Tooling</h1>
            <p className={styles.subtitle}>
              Automate your infrastructure and operations with comprehensive automation and tooling services. From CI/CD pipelines to network automation, reduce manual effort and increase reliability.
            </p>
            <p className={styles.description}>
              Automation is essential for modern IT operations at scale. We build production-grade automation systems that reduce manual effort, eliminate errors, and accelerate delivery across CI/CD, infrastructure, and network operations.
            </p>
          </div>
        </div>
      </section>
          ...
          <div className={styles.caseStudiesHeader}>
            <div className={styles.caseStudiesTitleWrapper}>
              ...
              <p className={styles.sectionSubtitle}>
                CI/CD pipelines, infrastructure automation, and developer tooling implementations.
              </p>
            </div>
      ...
      <section className={styles.technicalAssistant}>
        <div className="container-custom">
          <RAGWidget contextId="services:automation-tooling" />
        </div>
      </section>
    </main>
  )
