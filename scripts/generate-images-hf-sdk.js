/**
 * Image Generation Script using Hugging Face Inference SDK
 * This uses the official @huggingface/inference package
 */
...
const config = {
  ...
  model: 'stabilityai/stable-diffusion-xl-base-1.0', // Higher quality model
  width: 1024,
  height: 576
};

async function generateImage(hf, prompt, filepath, retries = 3) {
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      ...
      if (error.message.includes('503') || error.message.includes('loading') || error.message.includes('unavailable')) {
        const waitTime = 30000 * (attempt + 1); // Exponential backoff: 30s, 60s, 90s
...
async function main() {
  ...
  if (!token) {
    console.error('Error: Hugging Face API token required');
    console.error('Get a free token at: https://huggingface.co/settings/tokens');
    console.error('Usage: node scripts/generate-images-hf-sdk.js --token YOUR_TOKEN');
    console.error('Or set HUGGINGFACE_API_TOKEN environment variable');
    process.exit(1);
  }

  const hf = new HfInference(token);

  console.log(`Using Hugging Face model: ${config.model}`);
  ...
  for (const [category, images] of Object.entries(imagePrompts)) {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      try {
        ...
        await generateImage(hf, image.prompt, fullPath);
        console.log(`  ✓ Generated: ${image.filename}`);
        successCount++;
        
        // Rate limiting: wait between images (free tier: ~30 requests/minute)
        ...
        // Wait before next image even on failure
